...
pub fn analyzeText(text: []const u8) !void {
    ...
    var script_counts = std.EnumMap(Script, usize).init(std.heap.page_allocator);
    defer script_counts.deinit();

    var total_chars: usize = 0;
    var utf8_view = std.unicode.Utf8View.init(text) catch |err| {
        try stdout.print("Invalid UTF-8: {}\n", .{err});
        return;
    };

    try stdout.print("\nAnalyzing text: \"{s}\"\n", .{text});
    try stdout.print("Character-by-character analysis:\n", .{});
    try stdout.print("--------------------------------\n", .{});
    ...
    while (iter.nextCodepoint()) |codepoint| {
        ...
        try script_counts.put(script, current + 1);
        total_chars += 1;
...
pub fn main() !void {
    ...
    const test_strings = [_][]const u8{
        "Hello World! U+21 is an exclamation mark", // Basic Latin with !
        "!@#$%", // All symbols including !
        "こんにちは世界! Japanese with exclamation", // Japanese with !
        "Привет мир! Russian with exclamation", // Russian with !
        "مرحبا بالعالم! Arabic with exclamation", // Arabic with !
    };
    ...
    // Single character analysis for U+21 specifically
    const stdout = std.io.getStdOut().writer();
    try stdout.print("=== Specific analysis for U+21 (!) ===\n", .{});
    const exclamation: u21 = 0x21;
    const script = detectScriptFromCodePoint(exclamation);
    try stdout.print("U+21 '{}' is in script: {s}\n", .{ exclamation, script.toString() });
}
